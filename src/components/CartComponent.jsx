...
const CartComponent = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      try {
        const response = await axios.get("https://34.201.92.59/carts", {
          headers: {
  ...
  const handleQuantityChange = async (itemId, newQuantity) => {
    ...
    try {
      await axios.put(
        "https://34.201.92.59/carts/update-cart",
        {
  ...
  const handleRemoveItemFromCart = async (productId) => {
    try {
      ...
      const response = await axios.delete(
        "https://34.201.92.59/carts/remove-item",
        {
  ...
  const handleCheckout = async () => {
    try {
      ...
      const checkoutSession = await axios.post(
        "https://34.201.92.59/orders/create-checkout-session",
