...
const CardComponent = ({ productName, id, price, info, image }) => {
    ...
    return (
        <div id={id} onClick={() => handleClick()} className="w-full min-w-60 max-w-80 min-h-[28rem]  border  rounded-lg shadow bg-gray-800 border-gray-700">
            <a href={`/product/${id}`}>
                <img className="p-8 rounded-t-lg aspect-square object-cover" src={image || defaultImage} alt="product image" />
            </a>
            <div className="px-5 pb-5">
                <a href={`/product/${id}`}>
                    <h5 className="text-xl font-semibold tracking-tight  text-white min-h-14">{productName}: {info}</h5>
                </a>
                ...
                <div className="flex flex-col gap-y-4 text-end items-center justify-end">
                    <span className="text-2xl font-bold text-white">
                        ${price}</span>
                    <a href="#" className=" text-white focus:ring-4 focus:outline-none  font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-blue-600 hover:bg-blue-700 focus:ring-blue-800">Add to cart</a>
                </div>
            </div>
        </div>
    )
