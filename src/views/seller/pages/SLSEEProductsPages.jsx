  ...
  return (
    <div className="min-h-screen bg-gray-100 flex justify-center items-center px-6 py-8">
      <div className="w-full max-w-6xl bg-white rounded-lg shadow-xl p-8">
              ...
              {products.map((product) => (
                <tr key={product._id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <Link to={`/admin/products/edit/${product._id}`} className="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-4 rounded mr-2">Actualizar</Link>
                    <Link to={`/admin/products/delete/${product._id}`} className="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-4 rounded">Eliminar</Link>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {products.length === 0 && <div className="text-center py-4">No se encontraron productos.</div>}
      </div>
    </div>
  );
}
